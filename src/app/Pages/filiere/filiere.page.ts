...
export class FilierePage implements OnInit {
  ...
  constructor(private modalService: NgbModal, private filiereservice: FiliereService,
  ...
  SearchFiliere() {
    if (this.nom != "") {
      this.listFiliere = this.listFiliere.filter(res => res.nom.toLowerCase().match(this.nom.toLowerCase()));
    }
    else if (this.nom == "") {
      this.getFiliereByUniversiteId();
    }
