...
@Component({
  ...
  templateUrl: './table-enseignant.page.html',
  styleUrls: ['./table-enseignant.page.scss'],
})
export class TableEnseignantPage implements OnInit {
  ...
  SearchEnseignant() {
    if (this.nom != '') {
      this.ListEnseignant = this.ListEnseignant.filter(res => res.nom.toLowerCase().match(this.nom.toLowerCase()));
    }
    else if (this.nom == '') {
      this.getAllEnseignantByUniversiteId();
    }
