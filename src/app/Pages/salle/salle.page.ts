...
export class SallePage implements OnInit {
  ...
  constructor(private salleservice: SalleService, private modalService: NgbModal,
  ...
  SearchSalle() {
    if (this.nom != '') {
      this.listSalle = this.listSalle.filter(res => res.bloc.toLowerCase().match(this.nom.toLowerCase()));
    }
    else if (this.nom == '') {
      this.getSalleByUniversiteId();
    }
