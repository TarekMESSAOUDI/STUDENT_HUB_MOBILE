...
@Component({
  ...
  templateUrl: './table-etudiant.page.html',
  styleUrls: ['./table-etudiant.page.scss'],
})
export class TableEtudiantPage implements OnInit {
  ...
  SearchEtudiant() {
    if (this.nom != '') {
      this.ListEtudiant = this.ListEtudiant.filter(res => res.nom.toLowerCase().match(this.nom.toLowerCase()));
    }
    else if (this.nom == '') {
      this.getEtudiantByClass();
    }
