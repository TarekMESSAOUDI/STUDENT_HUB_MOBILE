...
export class SpecialitePage implements OnInit {
  ...
  constructor(private modalService: NgbModal, private specialiteservice: SpecialiteService,
  ...
  SearchSpecialite() {
    if (this.nom != '') {
      this.listSpecialite = this.listSpecialite.filter(res => res.nom.toLowerCase().match(this.nom.toLowerCase()));
    }
    else if (this.nom == '') {
      this.getSpecialiteByFiliereId();
    }
