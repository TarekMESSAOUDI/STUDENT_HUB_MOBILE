...
@Component({
  selector: 'app-niveau',
  templateUrl: './niveau.page.html',
  styleUrls: ['./niveau.page.scss'],
})
export class NiveauPage implements OnInit {
  ...
  constructor(private modalService: NgbModal, private niveauservice: NiveauService,
  ...
  SearchNiveau() {
    if (this.nom != '') {
      this.listNiveau = this.listNiveau.filter(res => res.nom.toLowerCase().match(this.nom.toLowerCase()));
    }
    else if (this.nom == '') {
      this.getNiveauBySpecialiteId();
    }
