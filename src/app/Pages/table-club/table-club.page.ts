...
@Component({
  ...
  templateUrl: './table-club.page.html',
  styleUrls: ['./table-club.page.scss'],
})
export class TableClubPage implements OnInit {
  ...
  SearchClubByName() {
    if (this.nom != '') {
      this.ListClub = this.ListClub.filter(res => res.nom.toLowerCase().match(this.nom.toLowerCase()));
    }
    else if (this.nom == '') {
      this.getAllClubsByUniversiteId();
    }
